...
```powershell
code --user-data-dir .tmp/vscode-profile --extensions-dir .tmp/vscode-extensions --install-extension codexvs-0.2.1-pre-release.vsix
```
...
**Max** is maximum single-agent reasoning. **Ultra (VS Code)** appears only when the same model advertises both `max` and `ultra`. CodexVS never sends raw `ultra` to app-server: it sends `max` and, when the actual request includes VS Code's native `runSubagent` or `agent/runSubagent` tool, adds proactive instructions naming that tool's exact `vscode_*` alias.
...
CodexVS accepts parseable stable Codex CLI releases from 0.144.4 onward. Older, malformed, and prerelease versions fail before launch. A CLI newer than the latest version validated for a given extension release produces a non-blocking warning.
...
- `codexvs.appServer.command`: machine-scoped Codex executable; defaults to `codex`.
- `codexvs.instructions`: developer instructions added to passive threads.
- `codexvs.model`: preferred discovered model.
- `codexvs.disabledModels`: discovered model IDs to hide.
- `codexvs.modelAliases`: obsolete-to-current model aliases.
- `codexvs.defaultReasoningEffort`: `auto` or a live catalog reasoning identifier; the native per-chat control wins.
- `codexvs.defaultServiceTier`: preferred advertised service tier.

VS Code's `chat.utilityModel` and `chat.utilitySmallModel` are global editor settings, not CodexVS settings. Use **Codex: Configure VS Code Utility Models** to set them.
