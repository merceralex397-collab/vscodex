...
function checkManifest(packageJson) {
  reject(packageJson.name !== 'vscodex', 'Package name must be vscodex.');
  reject(packageJson.displayName !== 'vsCodex', 'Display name must be vsCodex.');
  ...
  reject(packageJson.license !== 'MIT', 'vsCodex must declare the MIT source license.');
  ...
  reject(packageJson.repository?.url !== 'https://github.com/merceralex397-collab/vscodex.git',
    'Repository URL is not the vsCodex repository.');
  reject(packageJson.icon !== 'assets/vscodex.png', 'Manifest must use the original vsCodex icon.');
  ...
  reject(Object.keys(properties).some((key) => !key.startsWith('vsCodex.')),
    'Every extension setting must use the vsCodex namespace.');
  const forbiddenSettings = [
    'vsCodex.baseURL',
    'vsCodex.clientVersion',
    'vsCodex.credentialsSource',
    'vsCodex.transport',
    'vsCodex.maxOutputTokens',
    'vsCodex.modelPricingUsdPerMTok',
    'vsCodex.apiKey'
  ];
  ...
  const commandSetting = properties['vsCodex.appServer.command'];
  ...
  reject([...commandIds].some((command) => !command.startsWith('vsCodex.')),
    'Every command must use the vsCodex namespace.');
  ...
  reject(packageJson.contributes?.languageModelChatProviders?.[0]?.vendor !== 'vscodex',
    'Language model provider vendor must be vscodex.');
...
async function checkBundle(filePath) {
  ...
  const forbidden = [
    ...
    ['API-key command', /vsCodex\.(?:setApiKey|clearApiKey)/],
    ['custom backend setting', /vsCodex\.baseURL/],
    ['transport selection setting', /vsCodex\.transport/],
