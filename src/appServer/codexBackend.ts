  ...
  constructor(
    ...
    this.disposables.push(
      ...
      vscode.workspace.onDidChangeConfiguration((event) => {
        if (event.affectsConfiguration('vsCodex.appServer.command')) {
  ...
  private logDiagnostic(event: AppServerDiagnosticEvent): void {
    ...
    if (event.kind === 'runtime-validated'
      ...
      void vscode.window.showWarningMessage(
        `Codex CLI ${event.runtimeVersion} is newer than ${LATEST_VALIDATED_CODEX_CLI_VERSION}, the latest version validated for this vsCodex release. Compatibility will be checked as features are used.`
      );
    }
  }
