...
export const MCP_DISABLED_TRANSPORT_COMMAND = './.vscodex-disabled-mcp';
...
export class CodexMcpIsolationError extends CodexRuntimeError {
  constructor(
    ...
    message = 'vsCodex could not prove that every configured MCP server is disabled.',
    options?: ErrorOptions
  ) {
...
export function createRuntimeDiagnostic(
  ...
  if (runtimeError.kind === 'unsupported-version') {
    ...
    if (reason === 'timeout') {
      detail = `vsCodex timed out while reading the redacted MCP list. Confirm \`codex mcp list\` completes on this extension host, then retry. Your global MCP configuration was not changed.${hostGuidance}`;
    } else if (reason === 'malformed') {
      detail = `vsCodex could not safely parse the redacted plain-text MCP list. Update Codex CLI or repair invalid MCP configuration, then retry. Valid global MCP servers do not need to be removed.${hostGuidance}`;
      ...
      detail = `vsCodex could not prove that every discovered MCP server was disabled for its passive app-server child. Update Codex CLI and retry; your global MCP configuration was not changed.${hostGuidance}`;
    } else {
