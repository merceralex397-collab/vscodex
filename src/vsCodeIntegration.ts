...
export class VsCodeIntegrationAdvisor implements ProviderIntegrationAdvisor {
  ...
  onModelSelected(): void {
    ...
    void vscode.window.showWarningMessage(
      ...
      if (action === CONFIGURE_ACTION) {
        return vscode.commands.executeCommand('vsCodex.configureUtilityModels');
      }
      return undefined;
    });
  }
  ...
  async configureUtilityModels(): Promise<void> {
    const models = await vscode.lm.selectChatModels({ vendor: 'vscodex' });
    if (models.length === 0) {
      await vscode.window.showErrorMessage(
        'No vsCodex model is currently available. Sign in to ChatGPT and retry.'
      );
      return;
    }
  ...
  async showDiagnostics(backend: CodexAppServerBackend): Promise<void> {
    ...
    const [models, tools] = await Promise.all([
      vscode.lm.selectChatModels({ vendor: 'vscodex' }),
...
export function readReasoningEffortSetting(): ReasoningEffortSetting {
  const configured = vscode.workspace.getConfiguration('vsCodex')
