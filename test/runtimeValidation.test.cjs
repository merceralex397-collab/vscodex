...
void (async () => {
  ...
  try {
    ...
    assert.deepEqual(disableArguments, [
      '-c',
      `mcp_servers={${JSON.stringify(punctuatedName)}={enabled=false,command="./.codexvs-disabled-mcp"}}`
    ]);
    assert.deepEqual(runtime.createMcpDisableArguments([
      ...
      '-c',
      'mcp_servers={"alpha"={enabled=false,command="./.codexvs-disabled-mcp"},"beta"={enabled=false,url="http://127.0.0.1:0/"}}'
    ]);
