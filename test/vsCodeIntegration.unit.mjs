...
const temporaryDirectory = await mkdtemp(path.join(os.tmpdir(), 'vscodex-integration-'));
...
const updates = [];
const models = [
  { vendor: 'vscodex', id: 'codex::large', name: 'Large', family: 'gpt-large' },
  { vendor: 'vscodex', id: 'codex::mini', name: 'Mini', family: 'gpt-mini' }
];
const backendModels = [{
  id: 'catalog-large',
  model: 'gpt-large',
...
try {
  ...
  settings.set('vsCodex.defaultReasoningEffort', 'future-mode');
  assert.equal(
    readReasoningEffortSetting(),
    'future-mode',
    'The global setting accepts future catalog identifiers.'
  );
  settings.set('vsCodex.defaultReasoningEffort', 'auto');
  assert.deepEqual(
  ...
  assert.deepEqual(updates, [
    { section: 'chat', key: 'utilityModel', value: 'vscodex/codex::large', target: 1 },
    { section: 'chat', key: 'utilitySmallModel', value: 'vscodex/codex::mini', target: 1 }
  ]);
  ...
  assert.equal(JSON.stringify(logs).includes('vscodex/codex::large'), false);
